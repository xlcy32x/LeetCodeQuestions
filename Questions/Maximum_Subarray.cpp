...
public:
    ...
    {
        std::cout<<"Result : "<<maxSubArray(nums)<<std::endl;
    }
};

int main()
{
    SolutionBase<Solution> * solution = new Solution;
    solution->Do(std::vector<int>{-2, 1, -3, 4, -1, 2, 1, -5, 4});
    solution->Do(std::vector<int>{-2, 1});
    // std::cin.get();
}